...
/// <summary>
/// Registers <see cref="UniFiProtectClient"/> with the dependency-injection container, backed by
/// <c>IHttpClientFactory</c> for pooled, long-lived HTTP connections.
/// </summary>
public static class UniFiProtectClientServiceCollectionExtensions
{
    private const string HttpClientName = "UniFi.Protect.Client";

    /// <summary>
    /// Register <see cref="UniFiProtectClient"/> as a typed <c>HttpClient</c> using the given options.
    /// TLS certificate pinning / trust settings from the options are applied to the primary handler.
    /// </summary>
    public static IHttpClientBuilder AddUniFiProtectClient(this IServiceCollection services, ProtectClientOptions options)
    {
        ArgumentNullException.ThrowIfNull(options);
        return services.AddUniFiProtectClient(_ => options);
    }

    /// <summary>
    /// Register <see cref="UniFiProtectClient"/> as a typed <c>HttpClient</c>, resolving its options
    /// from the service provider (e.g. from configuration or another registered service).
    /// </summary>
    public static IHttpClientBuilder AddUniFiProtectClient(
        this IServiceCollection services,
        Func<IServiceProvider, ProtectClientOptions> optionsFactory)
    {
        ...
        return services
            ...
            .AddTypedClient<UniFiProtectClient>((httpClient, sp) => new UniFiProtectClient(optionsFactory(sp), httpClient));
    }
